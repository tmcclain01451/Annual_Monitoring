...
if st.button("🚀 Run Analysis"):
    ...
    for table_label in selected_tables:
        ...
        for preserve, df_p in csv_df.groupby("Preserve"):
            ...
            # Support modules that produce dynamic, data-driven columns
        ...
        # Any preserve missing a species/status this year gets 0, not blank
        ...
        # Save cumulative per-preserve file for THIS table type
        for _, row in result_df.iterrows():
            ...
            try:
                ...
                st.error(
                    f"⚠️ Could not update **{preserve}** ({table_label}): "
                    f"file may be open in Excel. Skipping and continuing."
                )
            except Exception as e:
                st.error(
                    f"⚠️ Unexpected error updating **{preserve}** ({table_label}): {e}"
                )

        # Save/append master file for THIS table type
        ...
        try:
            ...
            st.error(
                f"⚠️ Could not update master file for {table_label}: "
                f"file may be open in Excel. Skipping and continuing."
            )
        except Exception as e:
            st.error(
                f"⚠️ Unexpected error updating master file for {table_label}: {e}"
            )
    ...
    for graph_label in selected_graphs:
        ...
        for preserve in sorted(csv_df["Preserve"].unique()):
            ...
            if os.path.exists(history_file):
                ...
                fig = module.build_graph(history_df, preserve)
                st.pyplot(fig)
        ...
        for preserve in sorted(csv_df["Preserve"].unique()):
            ...
            if os.path.exists(history_file):
                ...
                try:
                    ...
                    st.error(f"⚠️ Could not generate PDF for {preserve} ({report_label}): {e}")
            else:
                st.caption(
                    f"No history file yet for {preserve} ({source_prefix}) — "
                    f"run that table type first."
